...
function encodeLineToBytes(game, line, dictionaryMap, context, config) {
	...
	while (i < line.length) {
			...
			if (dictEntry) {
				...
				if (stemEntry && trailingSTokenHi <= config.tokenHiByteMax) {
					...
					for (const letter of rawWord) {
						bytes.push(mapGlyph(game, letter, context));
					}
				}
		...
		bytes.push(mapGlyph(game, ch, context));
		i += 1;
	}
...
function emitGameAsm(game, config) {
	...
	lines.push("; Generated by _util/dict_process.js");
	lines.push("; Passage bytes: $00-$3f are ASCII offsets ($20-$5f).");
	...
	lines.push(`; raw_bytes=${game.rawBytes} encoded_bytes=${game.encodedBytes} replaced_words=${game.replacedWords} replaced_words_with_space=${game.replacedWordsWithTrailingSpace} replaced_words_with_period=${game.replacedWordsWithTrailingPeriod} replaced_words_with_exclaim=${game.replacedWordsWithTrailingExclaim} replaced_words_with_question=${game.replacedWordsWithTrailingQuestion} replaced_words_with_s=${game.replacedWordsWithTrailingS} replaced_words_with_r=${game.replacedWordsWithTrailingR}`);
	lines.push("");
	return lines.join("\n");
}
